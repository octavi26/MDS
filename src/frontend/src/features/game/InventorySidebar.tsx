      ...
      <div className="p-4 border-b border-zinc-800 bg-zinc-900/50">
        ...
        <div className="relative group">
          ...
          <input 
            ...
            className="w-full bg-zinc-800 border border-zinc-700 rounded-md py-2 pl-10 pr-4 text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all"
          />
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 gap-2">
          {items.length > 0 ? (
            items.map((item, index) => (
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-zinc-700 flex items-center justify-center text-lg mr-3 shadow-inner group-hover:bg-zinc-600 transition-colors">
                  {/* Item Icon Placeholder - could use emojis or small icons later */}
                  ✨
                </div>
            ...
            <div className="flex flex-col items-center justify-center h-full opacity-30 text-center py-12">
              <p className="text-sm italic">No items discovered yet</p>
            </div>
          )}
        </div>
      </div>
